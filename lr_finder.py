...
class LRFinder(object):
    ...
    def range_test(
        ...
        for iteration in tqdm(range(num_iter)):
            ...
            hidden_states = self.model.init_hidden_states(inputs.shape[0])

            # Train on batch and retrieve loss
            loss, hidden_states = self._train_batch(inputs, labels, hidden_states)
            if val_loader:
    ...
    def _train_batch(self, inputs, labels, hidden_states):
        ...
        outputs, hidden_states = self.model(inputs, hidden_states)
        loss = self.criterion(outputs.squeeze(), labels.float())

        # Backward pass
        loss.backward()
        
        # Clip the norm of the gradient to prevent exploding gradient
